<template>
  <div class="row" id="pdfDom" style="padding: 55px 16px;;background-color: #fff;">
    ...
    <div v-if="tableFlag && datas.length !== 0" style="position: relative;left: 0;top: 0;overflow-x: auto;">
      ...
      <div style="padding-bottom: 4px;white-space: nowrap"><el-tag size="small ">{{$t('swineReport.farm')}}:{{farmName}}&nbsp;{{tableTh.farm_org_name_loc}}</el-tag><el-tag size="small " style="margin-right: 5px;padding: 0 15px;">{{$t('swineReport.breedGroup')}}: {{startGroup}}-{{endGroup}}</el-tag><el-tag size="small ">{{$t('swineReport.printTime')}}:{{tableTh.printReportTime}}</el-tag></div>
...
//            let mytop = jQuery(this).scrollTop() - 252
//            console.log(mytop)
