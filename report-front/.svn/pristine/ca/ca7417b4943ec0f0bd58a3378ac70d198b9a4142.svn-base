<template>
  <div class="row" id="pdfDom" style="padding: 55px 17px;background-color:#fff;">
        ...
        <div class="form-group col-xs-12 col-sm-6">
          <div class="input-group col-xs-12 col-sm-12" style="padding: 0 15px;">
            ...
            <div class="form-control" style="padding: 0; bottom: 0px">
              <el-input-number style="width: 100%;"v-model="PAR_Max_PARITY" :min="0" :max="12" size="small"></el-input-number>
            </div>
          </div>
        </div>
        <div class="form-group col-xs-12 col-sm-6">
    ...
    <div v-show="tableFlag && datas.length !== 0" style="">
      <table id="fHeader" class="table table-bordered">
        <thead v-show="device=='PC'&& fixedHeader">
        <tr>
          <th>{{$t('swineReport.breakDown')}} </th>
          <th>{{tableTh.parity_01_show}}</th>
          <th>{{tableTh.parity_02_show}}</th>
          <th>{{tableTh.parity_03_show}}</th>
          <th>{{tableTh.parity_04_show}}</th>
          <th>{{tableTh.parity_05_show}}</th>
          <th>{{tableTh.parity_06_show}}</th>
          <th>{{tableTh.parity_07_show}}</th>
          <th>{{tableTh.parity_08_show}}</th>
      ...
      <h4 style="text-align: center">{{$t('message.titleSwineReport07')}}</h4>
      ...
      <table v-show="device=='PC'" id="sTable" class="table table-bordered table-hover table-striped">
        ...
        <tr v-for="data in datas">
          <!--<td>{{(data.sub_category_name_loc) == ' '?'':data.sub_category_name_loc}}</td>-->
          <td>
            {{(language === 'en')===true?data['sub_category_name_eng']:data['sub_category_name_loc']}}
          </td>
        ...
        <tr v-for="data in datas">
          <td>
            <div class="card-views">
              ...
              <div class="card-view">
                <span class="title"> {{tableTh.parity_01_show}}</span>
                <span class="value">{{data.column_1}} </span>
              </div>
              <div class="card-view">
                <span class="title"> {{tableTh.parity_02_show}}</span>
                <span class="value"> {{data.column_2}} </span>
              </div>
              <div class="card-view">
                <span class="title"> {{tableTh.parity_03_show}} </span>
                <span class="value"> {{data.column_3}} </span>
              </div>
              <div class="card-view">
                <span class="title"> {{tableTh.parity_04_show}} </span>
                <span class="value"> {{data.column_4}} </span>
              </div>
              <div class="card-view">
                <span class="title"> {{tableTh.parity_05_show}} </span>
                <span class="value"> {{data.column_5}} </span>
              </div>
              <div class="card-view">
                <span class="title"> {{tableTh.parity_06_show}} </span>
                <span class="value"> {{data.column_6}} </span>
              </div>
              <div class="card-view">
                <span class="title"> {{tableTh.parity_07_show}} </span>
                <span class="value"> {{data.column_7}} </span>
              </div>
              <div class="card-view">
                <span class="title"> {{tableTh.parity_08_show}} </span>
  ...
  export default {
    ...
    data () {
      return {
        ...
        up: false,
        tableTh: {},
        htmlTitle: this.$t('message.titleSwineReport07')
      }
    },
    mounted () {
      this.headerMonitor()
    ...
    methods: {
      ...
      doSearch () {
        ...
        _self.loadFlag = true
        jQuery.ajax({
          url: '/standard/' + _self.orgCode + '/' + _self.farmOrg + '/report7',
          type: 'POST',
          // contentType: 'application/json',
          data: {'PAR_DATE': _self.PAR_DATE, 'PAR_ALL_BREED': _self.PAR_ALL_BREED, 'PAR_START_BREED': _self.PAR_START_BREED, 'PAR_END_BREED': _self.PAR_END_BREED, 'PAR_BACK_PERIOD_WEEKS': _self.PAR_BACK_PERIOD_WEEKS, 'pAR_REPORT_TYPE': _self.PAR_REPORT_TYPE, 'PAR_Max_PARITY': _self.PAR_Max_PARITY},
          dataType: 'json',
