<template>
  <div class="row" id="pdfDom" style="padding: 55px 16px;background-color: #fff;">
    ...
    <div style="" v-if="tableFlag && datas.length !== 0">
      ...
      <div style="padding-bottom: 4px">
        <el-tag size="small ">{{$t('swineReport.farm')}}:{{tableTh.farm_org}}&nbsp;{{tableTh.farm_name_loc}}</el-tag>
...
<script>
  ...
  export default {
    ...
    methods: {
      getExcel () {
        let excelTitle = '<h1 style="text-align: center">' + this.tableTh.org_name_loc + '</h1>'
      ...
      getPoint () {
        let top = window.document.body.scrollTop
        console.log(top)
        if (this.up) {
          alert('A')
        } else {
...
//            this.tableFlag = false
      ...
      headerMonitor () {
        let self = this
//        let hHeight = jQuery('#sTable').offset().top
