  ...
  export default {
    ...
    methods: {
      getExcelDowload () {
        ...
        var secondaryHeadlines = _self.$t('message.titleSwineReport13')
        var reportName = _self.$t('message.titleSwineReport13')
        var reportDetail = []
        var columnName = []
        ...
        var params = {'reportName': reportName, 'title': title, secondaryHeadlines: secondaryHeadlines, reportType: 'repor13', 'picDateList': this.datas, 'imageBase64': this.chart.toImage(), 'columnName': columnName, reportDetail: reportDetail}
        jQuery.ajax({
          url: url,
          type: 'POST',
          ...
          success: function (response, status, request) {
            if (response.status === 'success') {
              ...
              n.href = '/static/8002013/' + _self.language + '/tempExcel/swineStandard/' + response.name
              n.click()
            } else {
              alert('导出失败')
            }
