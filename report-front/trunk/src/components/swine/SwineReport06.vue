  ...
  <div class="row" id="pdfDom" style="padding-top: 55px;background-color: #fff;">
    <form class="form" style="overflow: hidden;">
      <div v-show="up">
        ...
        <!--配种日期 分移动和pc-->
        <div class="form-group col-xs-12 col-sm-6" v-if="device=='PC2'">
          <div class="input-group col-xs-12 col-sm-12" style="padding: 0 15px">
            <div class="input-group-addon">{{$t('swineReport.breedingDate')}}: </div>
            <div class="form-control" style="padding: 0; bottom: 0px;height: auto;">
              <el-date-picker style="width: 100%;" size="small"v-model="daterangeVal" type="daterange" :range-separator="this.$t('swineReport.to')"
                              :start-placeholder="startDate" :end-placeholder="endDate"  :picker-options="pickerOptions2" value-format="yyyy-MM-dd">
              </el-date-picker>
            </div>
          </div>
        </div>
    ...
    <div v-show="tableFlag  && datas.length !== 0" style="padding: 0 25px;">
      <div style="padding-bottom: 4px">
        ...
        <el-tag size="small ">{{$t('swineReport.farm')}}: {{tag.farm_org }} {{tag.farm_name_loc}}</el-tag>
        <el-tag size="small ">{{$t('swineReport.breedingDate')}}: {{exStDate}} - {{exEnDate}}</el-tag>
      ...
      <table v-show="device=='PC'" id="tableContent" class="table table-bordered table-hover table-striped">
        <thead>
          <tr>
          <th>{{$t('swineReport.rangeOfDays')}} </th>
          <th>{{$t('swineReport.qty')}} </th>
        ...
        <tbody>
          ...
          <td>
            <div class="card-views">
              <div class="card-view">
                <span class="title"> {{$t('swineReport.rangeOfDays')}} </span>
    ...
    <div v-show="tableFlag" id="container" style="width:100%;margin:20px 0px 60px 0px"></div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        daterangeVal: [],
        htmlTitle: this.$t('message.titleSwineReport06'),
        exStDate: '',
        exEnDate: ''
      }
    },
    ...
    methods: {
      getExcelDowload () {
        var _self = this
        var reportName = _self.$t('swineReport.repeatService')
        var url = '/standard/' + _self.orgCode + '/' + _self.farmOrg + '/exportStandardPic'
        var reportDetail = []
        var columnName = []
        var secondaryHeadlines = _self.$t('swineReport.repeatService')
        var title = (this.language === 'en') === true ? this.datas[0].org_name_eng : this.datas[0].org_name_loc
        columnName.push(_self.$t('swineReport.rangeOfDays'))
        columnName.push(_self.$t('swineReport.qty'))
        columnName.push(_self.$t('swineReport.rate'))
        reportDetail.push(_self.$t('swineReport.farm') + ':' + _self.tag.farm_org + _self.tag.farm_name_loc)
        reportDetail.push(_self.$t('swineReport.breedingDate') + ':' + _self.exStDate + '-' + _self.exEnDate)
        reportDetail.push(_self.$t('swineReport.printTime') + ':' + _self.tag.printReportTime)
        var params = {reportName: reportName, 'title': title, secondaryHeadlines: secondaryHeadlines, reportType: 'repor6', 'picDateList': this.datas, 'imageBase64': this.chart.toImage(), 'columnName': columnName, reportDetail: reportDetail}
        jQuery.ajax({
          url: url,
          type: 'POST',
          ...
          success: function (response, status, request) {
            ...
            //  var n = document.createElement('a')
              ...
              n.href = '/static/8002006/' + _self.language + '/tempExcel/swineStandard/' + response.name
              n.click()
            } else {
              alert('导出失败')
            }
      ...
      getExcel () {
        var excelTitle = '<h1 style="text-align: center">' + this.tag.org_name_loc + '</h1>'
        excelTitle += '<h2 style="text-align: center">' + this.htmlTitle + '</h2>'
        excelTitle += '<div style="font-weight: bold"><span>' + this.$t('swineReport.farm') + ':' + this.tag.farm_org + '&nbsp;&nbsp;' + this.tag.farm_name_loc + '</span>'
